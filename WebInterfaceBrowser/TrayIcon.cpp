...
HWND CTrayIcon::GetMessageProcessorHWND()
{
	...
	if (!hWnd)
	{
        ...
        wc.lpszClassName = L"TRAY_ICON_MESSAGE_PROCESSOR_WND_CLASS";
		...
		hWnd = CreateWindowEx(
			0,
            L"TRAY_ICON_MESSAGE_PROCESSOR_WND_CLASS",
            L"TRAY_ICON_MESSAGE_PROCESSOR_WND",
			WS_POPUP,
			0, 0, 0, 0,
			NULL,
			NULL,
...
bool CTrayIcon::ShowBalloonTooltip(const std::wstring& title, const std::wstring& msg, ETooltipIcon icon)
{
    ...
    {
        int Size = sizeof(data.szInfoTitle)/sizeof(wchar_t);

        std::wstring Data = title;
        if(Data.length() + 1 > Size)
            Data.erase(Size - 1,Data.length() - Size + 1);

        wcscpy_s(data.szInfoTitle,Size,Data.c_str());
    }

    {
        int Size = sizeof(data.szInfo)/sizeof(wchar_t);

        std::wstring Data = msg;
        if(Data.length() + 1 > Size)
            Data.erase(Size - 1,Data.length() - Size + 1);

        wcscpy_s(data.szInfo,Size,Data.c_str());
    }
