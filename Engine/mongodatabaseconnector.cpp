...
namespace BrowserAutomationStudioFramework
{
    ...
    void MongoDatabaseConnector::SetDatabaseConnectionWindow(IDatabaseConnectionWindow * DatabaseConnectionWindow)
    {
        ...
        connect(this,SIGNAL(AddConnectionLog(QString)), DatabaseConnectionWindow,SLOT(AddConnectionLog(QString)));
    ...
    void MongoDatabaseConnector::ExportExcel(const QString& File, QList<int> ColumnIds, DatabaseMassSelector Selector)
    {
        ...
        try
        {
            ...
            std::auto_ptr<DBClientCursor> cursor = _c.query(QString("%1.res").arg(Selector.TableId).toStdString(), q, 0, 0, 0, 0, 3);
            if(cursor.get() == 0)
            {
                throw std::exception();
            }
            ...
            while(!in.atEnd())
            {
                ...
                foreach(QString Item, Items)
                {
                    ...
                    switch(Column.Type)
                    {
                        ...
                        case DatabaseColumn::Date:
                        {
                            ItemBuild.append(column_name,Date_t(QDateTime::fromString(Item,"yyyy-MM-dd HH:mm:ss").toMSecsSinceEpoch()));
                        }break;
            ...
            foreach(DatabaseFilter Filter,Filters.FilterList)
            {
                ...
                switch(Filter.Type)
                {
                    ...
                    case DatabaseFilter::StringContains:
                        on = BSON(c << BSON("$regex"<< QString(QString(".*") +  QRegExp::escape(Filter.Data.toString()) + QString(".*")).toStdString()));
                    break;
                    case DatabaseFilter::StringEquals:
                        on = BSON(c << QString(Filter.Data.toString()).toStdString());
                    break;
                    ...
                    case DatabaseFilter::BoolIsFalse:
                    {
                        ...
                        build.append(BSON(c << false));
                        build.append(BSON(c << BSON("$exists"<<false)));
                        on = BSON("$or" << build.arr());
                    }
                    break;
    ...
    DatabaseItem MongoDatabaseConnector::Select(const QString& Id, int TableId)
    {
        _WasError = false;
        DatabaseItem Res;
        try
        {
            ...
            if(!EnsureIndexTables.contains(TableId))
            {
                _c.eval(QString::number(TableId).toStdString(),"db.res.ensureIndex( { groups: 1 } )");
                EnsureIndexTables.insert(TableId);
            }
            ...
            Items.SelectedRows.insert(Id);

            BSONObj o = GenerateDatabaseItemsQuery(Items);
            //qDebug()<<o.toString().c_str();
            int limmit = 1;
            int offset = 0;

            Query q(o);
            std::auto_ptr<DBClientCursor> cursor = _c.query(QString("%1.res").arg(TableId).toStdString(), q, limmit, offset, 0, 0, 3);
            if(cursor.get() == 0)
            {
                throw std::exception();
            }

            QList<DatabaseColumn> Columns = GetColumns(TableId);
            ...
            while (cursor->more())
            {
               ...
               Res.Id = QString::fromStdString(obj.getField("_id").OID().toString());
               foreach(DatabaseColumn Column,Columns)
               {
                    std::string column = QString::number(Column.Id).toStdString();
                    switch(Column.Type)
                    {
                        ...
                        case DatabaseColumn::Int:
                            if(obj.getField(column).type() == mongo::NumberInt)
                                Res.Data[Column.Id] = obj.getIntField(column);
                            else
                                Res.Data[Column.Id] = 0;

                        break;
                        case DatabaseColumn::Date:

                            if(obj.getField(column).type() == mongo::Date)
                                Res.Data[Column.Id] = QDateTime::fromMSecsSinceEpoch(obj.getField(column).Date().asInt64());
                            else
                                Res.Data[Column.Id] = QDateTime(QDate(2000,1,1));
                        break;
                        case DatabaseColumn::Bool:

                            if(obj.getField(column).type() == mongo::Bool)
                                Res.Data[Column.Id] = obj.getBoolField(column);
                            else
                                Res.Data[Column.Id] = false;
                        break;
                    }
               }
               break;
    ...
    QList<DatabaseItem> MongoDatabaseConnector::Select(DatabaseSelector Selector)
    {
        ...
        try
        {
            ...
            std::auto_ptr<DBClientCursor> cursor = _c.query(QString("%1.res").arg(Selector.TableId).toStdString(), q, limmit, offset, 0, 0, 3);
            if(cursor.get() == 0)
            {
                throw std::exception();
            }
        ...
        try
        {
            ...
            while (i.hasNext())
            {
                ...
                switch(i.value().type())
                {
                    ...
                    {
                        ItemBuild.append(column,(int)i.value().toDouble());
                    }break;
    ...
    void MongoDatabaseConnector::Update(DatabaseItem Item, DatabaseMassSelector Selector)
    {
        try
        {
            BSONObjBuilder b;
            ...
            _c.update(QString("%1.res").arg(Selector.TableId).toStdString(),
                 o,
                BSON("$set" << b.obj()),false,true, NULL
            );
        }catch( const mongo::DBException &e ) {
            _WasError = true;
            QString error = QString::fromStdString(e.what());
    ...
    bool MongoDatabaseConnector::TryConnect()
    {
        ...
        try
        {
            ...
            if(ConnectionIsRemote)
            {
                ...
                emit AddConnectionLog("Remote Connection String = " + ConnectionString);
        ...
        } catch( const DBException &e )
        {
            ...
            emit AddConnectionLog("Connection Error = " + _ErrorString);
        }catch( ... )
        {
            IsDatabasePresent = false;
            _ErrorString = tr("Database error");
            ...
            emit AddConnectionLog("Unknown database error");
        }
    ...
    DatabaseFilters MongoDatabaseConnector::ParseFilter(const QString& JsonFilters)
    {
        ...
        for(QJsonValue val:InputObject.array())
        {
            ...
            if(array.size()>=2)
            {
                ...
                {
                    f.Type = DatabaseFilter::StringEquals;
                    if(IsInt)
                    {
                        f.Data = QString::number(IntValue);
                    }else if(IsString)
    ...
    DatabaseSelector MongoDatabaseConnector::ConvertToDatabaseSelector(const QVariant& Data)
    {
        DatabaseSelector Res;
        QVariantMap Map = Data.toMap();
        if(Map.contains("TableId"))
        {
            Res.TableId = Map["TableId"].toInt();
        }else
        {
            Res.TableId = 0;
        }
        ...
        {
            Res.Sort.IsNull = false;
            QVariantMap M = Map["Sort"].toMap();
            Res.Sort.ColumnId = M["ColumnId"].toInt();
            Res.Sort.OrderIsAsc = M["OrderIsAsc"].toBool();
        }else
        {
        ...
        if(Map.contains("Page"))
        {
            ...
            QVariantMap M = Map["Page"].toMap();
            Res.Page.PageNumber = M["PageNumber"].toInt();
            Res.Page.PageSize = M["PageSize"].toInt();
        }else
        {
            Res.Page.IsNull = true;
        }

        if(Map.contains("Groups"))
        {
            Res.Groups.IsNull = false;
            QVariantMap M = Map["Groups"].toMap();
            Res.Groups.GroupIdList = M["GroupIdList"].toStringList();
        }else
        {
            Res.Groups.IsNull = true;
        }

        if(Map.contains("Filter"))
        {
            Res.Filter.IsNull = false;
            QVariantMap M = Map["Filter"].toMap();
            for(QVariant& FilterVariant: M["FilterList"].toList())
            {
                QVariantMap FilterMap = FilterVariant.toMap();
                DatabaseFilter Filter;
                Filter.ColumnId = FilterMap["ColumnId"].toInt();

                QString ColumnType = FilterMap["Type"].toString();
                if(ColumnType == "StringNotEmpty")
                {
                    Filter.Type = DatabaseFilter::StringNotEmpty;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "StringEmpty")
                {
                    Filter.Type = DatabaseFilter::StringEmpty;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "StringContains")
                {
                    Filter.Type = DatabaseFilter::StringContains;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "StringEquals")
                {
                    Filter.Type = DatabaseFilter::StringEquals;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntEquals")
                {
                    Filter.Type = DatabaseFilter::IntEquals;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntNotEquals")
                {
                    Filter.Type = DatabaseFilter::IntNotEquals;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntGreaterOrEqual")
                {
                    Filter.Type = DatabaseFilter::IntGreaterOrEqual;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntGreater")
                {
                    Filter.Type = DatabaseFilter::IntGreater;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntLessThan")
                {
                    Filter.Type = DatabaseFilter::IntLessThan;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "IntLessThanOrEqual")
                {
                    Filter.Type = DatabaseFilter::IntLessThanOrEqual;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "BoolIsTrue")
                {
                    Filter.Type = DatabaseFilter::BoolIsTrue;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "BoolIsFalse")
                {
                    Filter.Type = DatabaseFilter::BoolIsFalse;
                    Filter.Data = FilterMap["Data"];
                }else if(ColumnType == "DateIsLessThan")
                {
                    Filter.Type = DatabaseFilter::DateIsLessThan;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }else if(ColumnType == "DateGreaterThan")
                {
                    Filter.Type = DatabaseFilter::DateGreaterThan;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }else if(ColumnType == "DateIsSameYear")
                {
                    Filter.Type = DatabaseFilter::DateIsSameYear;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }else if(ColumnType == "DateIsSameMonth")
                {
                    Filter.Type = DatabaseFilter::DateIsSameMonth;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }else if(ColumnType == "DateIsSameDay")
                {
                    Filter.Type = DatabaseFilter::DateIsSameDay;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }else if(ColumnType == "DateIsSameHour")
                {
                    Filter.Type = DatabaseFilter::DateIsSameHour;
                    QDateTime timestamp;
                    timestamp.setTime_t(FilterMap["Data"].toInt() / 1000);
                    Filter.Data = timestamp;
                }
                ...
                Res.Filter.FilterList.append(Filter);
            }
        }else
        {
