...
namespace BrowserAutomationStudioFramework
{
        ...
        IResourceController(parent)
    {
        ...
        //NoProtection,PrivateScriptEnterPassForUser,PrivateScriptUserEnterPass
        CompileType = "NoProtection";
    }


    bool XmlResourceController::GetCopySilent()
    {
        return CopySilent;
    }
    bool XmlResourceController::GetIsEnginesInAppData()
    {
        return IsEnginesInAppData;
    }
    QString XmlResourceController::GetCompileType()
    {
        return CompileType;
    }
    void XmlResourceController::SetCopySilent(bool CopySilent)
    {
        this->CopySilent = CopySilent;
    }
    void XmlResourceController::SetIsEnginesInAppData(bool IsEnginesInAppData)
    {
        this->IsEnginesInAppData = IsEnginesInAppData;
    }
    void XmlResourceController::SetCompileType(const QString& CompileType)
    {
    ...
    void XmlResourceController::SetScriptIcon(const QString & Icon)
    {
        this->ScriptIcon = Icon;
    }

    void XmlResourceController::SetIsCustomIcon(bool IsCustomIcon)
    {
        this->IsCutomIcon = IsCustomIcon;
    }
    ...
    void XmlResourceController::SetIntegrateScheduler(bool IntegrateScheduler)
    {
        this->IntegrateScheduler = IntegrateScheduler;
    }

    bool XmlResourceController::GetIntegrateScheduler()
    {
        return IntegrateScheduler;
    }

    void XmlResourceController::SetSingleInstance(bool SingleInstance)
    {
        this->SingleInstance = SingleInstance;
    }

    bool XmlResourceController::GetSingleInstance()
    {
    ...
    QString XmlResourceController::GetScriptIcon()
    {
        return ScriptIcon;
    }

    bool XmlResourceController::GetIsCustomIcon()
    {
        return IsCutomIcon;
    }
    ...
    void XmlResourceController::SetModuleInfo(const QString & ModuleInfo)
    {
        this->ModuleInfo = ModuleInfo;
    }

    void XmlResourceController::SetEmbeddedData(const QString & EmbeddedData)
    {
    ...
    QString XmlResourceController::GetEmbeddedData()
    {
        return EmbeddedData;
    }

    void XmlResourceController::SetModulesPreserve(const QList<IModuleManager::ModulePreserve>& Modules)
    {
        this->Modules = Modules;
    }

    QList<IModuleManager::ModulePreserve> XmlResourceController::GetModulesPreserve()
    {
        return Modules;
    }
    ...
    void XmlResourceController::FromViewToModel(IResources * resources, bool Clear)
    {
        ...
        //Validation
        {
            ...
            if(!FileName.isEmpty())
            {
                ...
                Validator.setDevice(&file);
            }else
            {
                Validator.addData(FileContent);
            }

            bool WasTitle = false;
            while (!Validator.atEnd() && !Validator.hasError())
            {
                QXmlStreamReader::TokenType token = Validator.readNext();

                if(Validator.name() == "BrowserAutomationStudioProject" && token == QXmlStreamReader::StartElement)
                {
                    WasTitle = true;
                }
            }
            ...
            if(!WasTitle)
            {
                ErrorString = tr("This file doesn't look like BAS project.");
                return;
            }
        ...
        while(!xmlReader.atEnd() && !xmlReader.hasError())
        {
            ...
            }else if(xmlReader.name() == "ModuleInfo" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
            ...
            }else if(xmlReader.name() == "Module" && token == QXmlStreamReader::StartElement)
            {
                ...
                if(!Name.isEmpty())
                {
                    IModuleManager::ModulePreserve Module = std::make_shared<IModuleManager::ModulePreserveClass>();
                    Module->Data = QByteArray::fromBase64(xmlReader.text().toString().toUtf8());
                    Module->Name = Name;
            ...
            {
                xmlReader.readNext();
                UnusedModules = xmlReader.text().toString().split(";");
            }else if(xmlReader.name() == "ScriptIcon" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                ScriptIcon = xmlReader.text().toString();
            }else if(xmlReader.name() == "IsCustomIcon" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                IsCutomIcon = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "HideBrowsers" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                HideBrowsers = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "IntegrateScheduler" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                IntegrateScheduler = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "SingleInstance" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                SingleInstance = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "CopySilent" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                CopySilent = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "IsEnginesInAppData" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                IsEnginesInAppData = xmlReader.text().toString() == "true";
            }else if(xmlReader.name() == "CompileType" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                CompileType = xmlReader.text().toString();
    ...
    void XmlResourceController::FromModelToView(IResources * resources, bool Clear)
    {
        ...
        xmlWriter.writeStartElement("Script");
            xmlWriter.writeCDATA(Script);
        xmlWriter.writeEndElement();

        xmlWriter.writeStartElement("ModuleInfo");
            xmlWriter.writeCDATA(ModuleInfo);
        xmlWriter.writeEndElement();
        ...
        xmlWriter.writeStartElement("Modules");
            for(IModuleManager::ModulePreserve Module:Modules)
            {
                xmlWriter.writeStartElement("Module");
                    xmlWriter.writeAttribute("Name", Module->Name);
                    xmlWriter.writeCDATA(QString::fromUtf8(Module->Data.toBase64()));
                xmlWriter.writeEndElement();
            }
        xmlWriter.writeEndElement();

        xmlWriter.writeStartElement("EmbeddedData");
            xmlWriter.writeCDATA(EmbeddedData);
        xmlWriter.writeEndElement();
        ...
        xmlWriter.writeTextElement("IntegrateScheduler",(IntegrateScheduler) ? "true" : "false");
        xmlWriter.writeTextElement("SingleInstance",(SingleInstance) ? "true" : "false");

        xmlWriter.writeTextElement("CopySilent",(CopySilent) ? "true" : "false");
        xmlWriter.writeTextElement("IsEnginesInAppData",(IsEnginesInAppData) ? "true" : "false");
        xmlWriter.writeTextElement("CompileType",CompileType);
