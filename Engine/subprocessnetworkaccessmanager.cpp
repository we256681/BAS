...
namespace BrowserAutomationStudioFramework
{
    ...
    void SubprocessNetworkAccessManager::Received(const QString& value)
    {
        ...
        while(!xmlReader.atEnd() && !xmlReader.hasError())
        {
            ...
            }else if(xmlReader.name() == "FindAllCache" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                Worker->SetAsyncResult(QScriptValue(xmlReader.text().toString()));
                emit FindAllCache();
            ...
            }else if(xmlReader.name() == "FindUrlByMask" && token == QXmlStreamReader::StartElement)
            {
                xmlReader.readNext();
                Worker->SetAsyncResult(QScriptValue(xmlReader.text().toString()));
                emit FindUrlByMask();
    ...
    void SubprocessNetworkAccessManager::SetAcceptLanguagePattern(const QString& pattern,const QString& callback)
    {
        QString WriteString;
        QXmlStreamWriter xmlWriter(&WriteString);
        xmlWriter.writeStartElement("SetAcceptLanguagePattern");
            xmlWriter.writeAttribute("pattern", pattern);
        xmlWriter.writeEndElement();
    ...
    void SubprocessNetworkAccessManager::FindUrlByMask(const QString& Mask, const QString& callback)
    {
        QString WriteString;
        QXmlStreamWriter xmlWriter(&WriteString);
        xmlWriter.writeTextElement("FindUrlByMask",Mask);

        Worker->SetScript(callback);
        Worker->SetFailMessage(tr("Timeout during ") + QString("FindUrlByMask"));
        Worker->GetWaiter()->WaitForSignal(this,SIGNAL(FindUrlByMask()), Worker,SLOT(RunSubScript()), Worker, SLOT(FailBecauseOfTimeout()));
        Worker->GetProcessComunicatorActual()->Send(WriteString);
    }
    ...
    void SubprocessNetworkAccessManager::FindAllCache(const QString& Mask, const QString& callback)
    {
        QString WriteString;
        QXmlStreamWriter xmlWriter(&WriteString);
        xmlWriter.writeTextElement("FindAllCache",Mask);

        Worker->SetScript(callback);
        Worker->SetFailMessage(tr("Timeout during ") + QString("FindAllCache"));
        Worker->GetWaiter()->WaitForSignal(this,SIGNAL(FindAllCache()), Worker,SLOT(RunSubScript()), Worker, SLOT(FailBecauseOfTimeout()));
        Worker->GetProcessComunicatorActual()->Send(WriteString);
    }
