...
namespace BrowserAutomationStudioFramework
{
    ...
    QList<QString> StringBox::GetSome(int MaxLength)
    {
        QList<QString> res;
        int len = Data.length();
        if(len > MaxLength)
    ...
    void StringBox::RemoveIndex(int index)
    {
        if(index<0 || index>=Data.length())
        {
            return;
        }
        QList<QPair<QString,StringBoxNode> >::iterator i = Data.begin();
        i += index;
        if(i != Data.end())
        {
            emit Removed(i->first);
            Erase(i);
        }
    }
    ...
    {
        for(QList<QPair<QString,StringBoxNode> >::iterator i = Data.begin();i!=Data.end();i++)
        {
            emit Removed(i->first);
        }
        Data.clear();
    ...
    int StringBox::Find(const QString& value)
    {
    ...
    QList<int> StringBox::FindByKeys(const QStringList& keys)
    {
        QList<QPair<QString,StringBoxNode> >::iterator i;
        QList<int> res;
