...
QString DatabaseSchemaEditorDialog::GetXml()
{
    ...
    for(Table &t:_Model.Tables)
    {
        ...
        xmlWriter.writeAttribute("id",QString::number(t.GetId()));
...
void DatabaseSchemaEditorDialog::TableChanged()
{
    ...
    if(ui->listWidget->currentRow() >= 0)
    {
        ...
        CurrentTableId = _Model.Tables[ui->listWidget->currentRow()].GetId();
        ui->listWidget_2->clear();
...
void DatabaseSchemaEditorDialog::EraseItem()
{
    ...
    for (int i = 0; i < ui->listWidget->selectedItems().size(); ++i)
    {
        ...
        while (it.hasNext())
        {
            if (it.next().GetId() == item->data(Qt::UserRole).toInt())
                it.remove();
        }
        delete item;
    }
...
void DatabaseSchemaEditorDialog::on_TableName_textEdited(const QString &arg1)
{
    ...
    while (it.hasNext())
    {
        if (it.next().GetId() == CurrentTableId)
        {
            Table t = it.value();
            t.Name = arg1;
            it.setValue(t);
