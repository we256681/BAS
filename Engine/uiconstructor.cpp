...
void UIConstructor::EditUnit(int index)
{
    ...
    DesignResourceWrapper * WrapperWidget = WidgetResource->findChild<DesignResourceWrapper *>();
    if(WrapperWidget)
    {
        WrapperWidget->Render();
    }
...
void UIConstructor::dropEvent(QDropEvent *event)
{
    ...
    if (event->mimeData()->hasFormat("application/x-dnduiconstructdata"))
    {
        ...
        if(offset < 0)
            offset = 0;
        if(offset >= Data[CurrentTab].length())
            offset = Data[CurrentTab].length() - 1;
        ...
        if(event->pos().x() >= ui->UnitList->pos().x() && event->pos().y() >= ui->UnitList->pos().y())
        {
            ...
            if(Layout->itemAt(i) == 0 || Layout->itemAt(i)->widget() == 0)
                return;
