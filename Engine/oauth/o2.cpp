    ...
    if (grantFlow_ == GrantFlowAuthorizationCode || grantFlow_ == GrantFlowImplicit) {
        ...
        if (useExternalWebInterceptor_) {
            ...
            if (!replyServer_->isListening()) {
                if (replyServer_->listen(QHostAddress::LocalHost, localPort_)) {
        ...
        Q_EMIT openBrowser(url.toString());
...
void O2::onVerificationReceived(const QMap<QString, QString> response) {
    ...
    if (grantFlow_ == GrantFlowAuthorizationCode) {
        // Save access code
        QString TempCode = response.value(QString(O2_OAUTH2_GRANT_TYPE_CODE));
        setCode(TempCode);
        ...
        TempCode = code();
        parameters.insert(O2_OAUTH2_GRANT_TYPE_CODE, TempCode);
