...
namespace BrowserAutomationStudioFramework
{
    ...
    SolverFactory::SolverFactory(QObject *parent) :
        ISolverFactory(parent),ManualSolver(0),AntigateSolver(0), DbcSolver(0),RucaptchaSolver(0),TwocaptchaSolver(0),CapMonsterSolver(0),XevilSolver(0)
    {
    }
    ...
    void SolverFactory::UsedXevil()
    {
        emit UsedSolver("Xevil");
    }
    ...
    void SolverFactory::FailedXevil()
    {
        emit FailedSolver("Xevil");
    }

    void SolverFactory::ReplaceSolver(const QString& name, ISolver* Solver)
    {
        if(Solver)
        {
            Replaced[name] = Solver;
        }else
        {
    ...
    ISolver* SolverFactory::GetSolver(const QString& name)
    {
        ...
        if(name == "capmonster" || name == "capmonsterimage" || name == "capmonsteraudio")
        {
            if(CapMonsterSolver == 0)
            {
                ...
                CapMonsterSolver->SetTimeout(1000);
                CapMonsterSolver->SetStartImmediate(true);
        ...
        if(name == "xevil")
        {
            if(XevilSolver == 0)
            {
                XevilSolver = new AntigateCaptchaSolver(this);
                XevilSolver->SetServer("");
                XevilSolver->SetMultipleIds(false);
                XevilSolver->SetTimeout(1000);
                XevilSolver->SetStartImmediate(true);
                XevilSolver->SetHttpClientFactory(HttpClientFactory);
                connect(XevilSolver,SIGNAL(Used()),this,SLOT(UsedXevil()));
                connect(XevilSolver,SIGNAL(Failed()),this,SLOT(FailedXevil()));
            }
            return XevilSolver;
        }
        if(name == "dbc")
