...
BROWSERAUTOMATIONSTUDIO_WAIT_ITERATION = 0
...
function wait_content_visible()
{
    ...
    }else
    {
        wait(tr("Failed to wait for content ") + JSON.stringify(text_success),function(){
            _set_result(false);
            waiter_create_match_path(_arguments()).script("_BAS_HIDE(BrowserAutomationStudio_IsVisible)(self)",function(){
...
function wait_css_visible()
{
    ...
    }else
    {
        ...
        wait(tr("Failed to wait for css ") + JSON.stringify(text_success), function(){
            _set_result(false);
            waiter_create_css_path(_arguments()).script("_BAS_HIDE(BrowserAutomationStudio_IsVisible)(self)",function(){
               _set_result(_result().indexOf("true")>=0);
            })
        },text_success,func);
    }
...
function wait()
{
    ...
    _do(function(i)
    {
        BROWSERAUTOMATIONSTUDIO_WAIT_ITERATION = i
...
function get_element_selector(selector, need_all)
{
    ...
    if(selector.indexOf(">NOTDEFINED>") >= 0)
    {
        fail(tr("Action 'Start Loop' has not yet been called."))
        return;
    }
    ...
    }else
    {
        ...
        wait(tr("Failed to wait for element ") + text_success, function(){
            if(BROWSERAUTOMATIONSTUDIO_WAIT_ITERATION == 8)
                _info(tr("Waiting for element ") + text_success)
            _set_result(false);
            get_element_selector(_arguments()).script("_BAS_HIDE(BrowserAutomationStudio_IsVisible)(self)",function(){
               _set_result(_result().indexOf("true")>=0);
            })
        },text_success,func);
    }
