...
function proxy_parse(proxy_string)
{
    ...
    var password = "";
    var IsHttp = true;
    var proxy_string_original = proxy_string

    if(proxy_string.length === 0)
    ...
    for(var i = 0;i<list.length;i++)
    {
        ...
        if(server.length == 0 && (el.indexOf(".")>=0 || el.toLowerCase() == "localhost"))
        {
            server = el
    ...
    {
        fail(tr("Failed to parse proxy") + ": '" + proxy_string_original + "'")
    }
...
function _user_data_folder()
{
    return Helper.user_data_folder();
}
