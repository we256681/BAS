...
{
    if(CURLINFO_SSL_DATA_IN != type && CURLINFO_SSL_DATA_OUT != type)
    {
        TraceAndFilter *_TraceAndFilter = (TraceAndFilter *)userp;
        QByteArray DataNew((const char *)data,size);

        if(_TraceAndFilter->TraceFilter->isEmpty() || DataNew.contains(_TraceAndFilter->TraceFilter->toUtf8()))
            _TraceAndFilter->Trace->append(DataNew);
    }
	return 1;
}
...
QtCUrl::QtCUrl() {
    ...
    _trace_and_filter.Trace = &Trace;
    _trace_and_filter.TraceFilter = &TraceFilter;
}
...
void QtCUrl::SetTraceFilter(const QString& TraceFilter)
{
    this->TraceFilter = TraceFilter;
    _trace_and_filter.TraceFilter = &(this->TraceFilter);
}
...
void QtCUrl::exec(Options& options)
{
    if(options.size() > 1)
    {
        if(!FileName.isEmpty())
...
void QtCUrl::setOptions(OptionItems& opt) {
    OptionItems defaults;
    ...
    if(DoTrace)
    {
        ...
        curl_easy_setopt(_curl, CURLOPT_DEBUGDATA, &_trace_and_filter);
    }

    OptionItemsIterator i(defaults);
