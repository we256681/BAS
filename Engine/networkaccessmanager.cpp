...
namespace BrowserAutomationStudioFramework
{
    ...
    void NetworkAccessManager::SetAcceptLanguagePattern(const QString& pattern,const QString& callback)
    {
        Worker->SetScript(callback);
        Worker->RunSubScript();
    }
    ...
    void NetworkAccessManager::FindUrlByMask(const QString& Mask, const QString& callback)
    {
        QRegExp MaskRegexp;
        MaskRegexp.setPatternSyntax(QRegExp::Wildcard);
        MaskRegexp.setPattern(Mask);
        ...
        foreach(QString key,keys)
        {
            if(MaskRegexp.exactMatch(key))
            {
                Worker->SetAsyncResult(QScriptValue(key));
                Worker->SetScript(callback);
                Worker->RunSubScript();
                return;
            }
    ...
    void NetworkAccessManager::FindAllCache(const QString& Mask, const QString& callback)
    {
        //Not implemented
        Worker->SetAsyncResult(QScriptValue(QString()));
        Worker->SetScript(callback);
        Worker->RunSubScript();
    }
