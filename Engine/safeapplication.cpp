...
namespace BrowserAutomationStudioFramework
{
    ...
    bool SafeApplication::notify(QObject * receiver, QEvent * event)
    {
        ...
        catch(std::exception& e)
        {
            ...
            if(Logger)
                Logger->Write(QString("[") + QTime::currentTime().toString("hh:mm:ss") + "]" + QString("Exception thrown:") + e.what() + receiver->metaObject()->className() + QString::number(event->type()), LogFail);
