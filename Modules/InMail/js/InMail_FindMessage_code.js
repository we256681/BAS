...
_call_function(_InMail.<%if(wait){%>wait<%}else{%>find<%}%>Message, {criteria: {<%= [["from", from], ['"!from"', notFrom], ["to", to], ['"!to"', notTo], ["subject", subject], ['"!subject"', notSubject], ["text", text], ['"!text"', notText], ["flags", flags], ['"!flags"', notFlags], ["since", since], ["before", before]].filter(function(el){return el[1] !== '""'}).map(function(el){return el[0] + ':(' + el[1] + ')'}).join(', ') %>}, sorts: {type: (<%= sortType %>), field: (<%= sortField %>)}, body: [<%= [['html', getTextHtml], ['plain', getTextPlain], ['raw', getTextRaw]].filter(function(el){return el[1]}).map(function(el){return "'" + el[0] + "'"}).join(', ') %>], headers: [<%= [['from', getFrom], ['to', getTo], ['subject', getSubject], ['raw', getRawHeader]].filter(function(el){return el[1]}).map(function(el){return "'" + el[0] + "'"}).join(', ') %>]<%if(getSize){%>, size: <%= getSize %><%}%><%if(getFlags){%>, flags: <%= getFlags %><%}%><%if(getDate){%>, date: <%= getDate %><%}%><%if(getAttachNames){%>, attachnames: <%= getAttachNames %><%}%><%if(getAttachments){%>, attachments: (<%= attachmentsMask %>)<%}%><%if(wait){%>, minResults: (<%= minResults %>), interval: ((<%= interval %>) * 1000)<%}%><%if(box!=='""'){%>, box: (<%= box %>)<%}%>, timeout: (<%if(wait){%>(<%= timeout %>) * 1000<%}else{%><%= timeout_value() || 60000 %><%}%>)})!
var messageData = _result_function();
<%if(getUid){%><%= saveUid %> = messageData["uid"];<%}%>
