...
cv::Mat QImageToCvMat( QImage *inImage )
{
    ...
    void ImageProcessingText(char *InputJson, ResizeFunction AllocateSpace, void* AllocateData, void* DllData, void* ThreadData, unsigned int ThreadId, bool *NeedToStop, bool* WasError)
    {
        ...
        Qt::AlignmentFlag VFlag = Qt::AlignTop;
        Qt::AlignmentFlag HFlag = Qt::AlignHCenter;

        if(ParamsList[0] == QString("center"))
        {
            VFlag = Qt::AlignVCenter;
            ParamsList.removeAt(0);
        }else if(ParamsList[0] == QString("top"))
        {
            VFlag = Qt::AlignTop;
            ParamsList.removeAt(0);
        }else if(ParamsList[0] == QString("bottom"))
        {
        ...
        }else if(ParamsList[0] == QString("left"))
        {
            HFlag = Qt::AlignLeft;
            ParamsList.removeAt(0);

        }else if(ParamsList[0] == QString("right"))
        {
            HFlag = Qt::AlignRight;
            ParamsList.removeAt(0);
        ...
        if(data->contains(Id))
        {
            ...
            painter.drawText(X,Y,W,H,VFlag | HFlag | Qt::TextWordWrap,Text);
        }
    }
