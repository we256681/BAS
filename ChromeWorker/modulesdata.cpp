...
ModulesDataList LoadModulesData(const std::string& Locale, const std::string& BASPid, ModulesDataList& UnusedModules)
{
    ...
    Folders.push_back("modules");
    Folders.push_back(std::string("external/") + BASPid);
    Folders.push_back("custom");
                ...
                try
                {
...

                    {
                        ...
                        for(picojson::value ActionValue: ActionsArray)
                        {
                            ...
                            ActionItem->DescriptionNoTranslation = ActionDescriptionObject["en"].get<std::string>();
