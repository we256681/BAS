function MultiSelect()
{
	...
	this.Update = function(clientX,clientY, Pressed)
	{
		if($('.modal.in').length == 0 && Pressed)
		{
			x2 = clientX + document.documentElement.scrollLeft;
			y2 = clientY + document.documentElement.scrollTop;
			if(y2 > document.documentElement.scrollHeight - 1)
				y2 = document.documentElement.scrollHeight - 1
			if(x2 > document.documentElement.scrollWidth - 1)
				x2 = document.documentElement.scrollWidth - 1
			if(y2 < 0)
				y2 = 0
			if(x2 < 0)
				x2 = 0
	...
	this.Init = function()
	{
		...
		$( document ).ready(function() {
			...
			$("html").mousedown(function(e) {
				...
				if($('.modal.in').length == 0 && e.button == 0)
        		{
        			...
        			if(target.hasClass("main") || target.hasClass("tool-margin") || (target.prop("tagName") == "HTML" && e.clientX < document.documentElement.offsetWidth) )
        			{
						...
						x1 = e.clientX + document.documentElement.scrollLeft;
						y1 = e.clientY + document.documentElement.scrollTop;
						x2 = x1;
						y2 = y1;
			...
			$("html").mousemove(function(e) {
				self.Update(e.clientX, e.clientY, e.button == 0)
			});
